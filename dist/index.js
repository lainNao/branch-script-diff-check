...
function run() {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            // 入力内容表示
            core.info('\n[input]');
            core.info(JSON.stringify(Object.assign({ repository: process.env.GITHUB_REPOSITORY, currentBranchName: ((_a = process.env.GITHUB_REF) === null || _a === void 0 ? void 0 : _a.split('/').pop()) || '' }, input), null, 2));
            // 実行
            const execInputScript = () => {
                return (0, child_process_1.execSync)(input.comparedScript).toString();
            };
            const currentBranchResult = execInputScript();
            (0, child_process_1.execSync)(`git fetch origin ${input.comparedBranchName} && git checkout ${input.comparedBranchName}`);
            ...
            const isResultSame = currentBranchResult === comparedBranchResult;
            core.setOutput('isResultSame', isResultSame.toString());
            core.setOutput('currentBranchResult', `"${currentBranchResult}"`);
            core.setOutput('comparedBranchResult', `"${comparedBranchResult}"`);
            core.info('\n[output]');
            core.info(JSON.stringify({
                isResultSame: isResultSame.toString(),
                currentBranchResult,
                comparedBranchResult
            }, null, 2));
